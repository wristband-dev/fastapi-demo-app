...
def start_db() -> Client:
    try:
        ...
        logger.info(f"Firebase app already initialized. Project ID: {app.project_id}")
    except ValueError: # No app initialized yet
        logger.info("Firebase app not initialized. Proceeding with initialization.")
        ...
        try:
            ...
            logger.info(f"Using mandatory project_id '{project_id}' from config.")
        ...
        if not emulator_host:
            ...
            logger.info("FIRESTORE_EMULATOR_HOST not set in environment. Attempting to construct from config.yml.")
            try:
                ...
                if hostname and db_port_from_config:
                    ...
                    logger.info(f"Constructed and set FIRESTORE_EMULATOR_HOST to '{emulator_host}' from config.yml.")
                else:
        ...
        logger.info(f"Firebase app initialization completed. Effective Project ID: {initialized_app.project_id}")
